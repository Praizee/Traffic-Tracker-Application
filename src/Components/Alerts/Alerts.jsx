...
const Alerts = ({ errorMessages, successMessages }) => {
  ...
  useEffect(() => {
    const errorMessagesArray = Array.isArray(errorMessages)
      ? errorMessages
      : [];
    const successMessagesArray = Array.isArray(successMessages)
      ? successMessages
      : [];
    ...
    if (combinedMessages.length > 0) {
      // Create alert based on both error and success messages
      const newAlerts = combinedMessages.map((message, index) => (
        <div
          key={index}
          className={`${
            errorMessagesArray.includes(message) ? "bg-red-500" : "bg-green-500"
